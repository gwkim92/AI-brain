...
export type ExternalRouteAction =
  ...
  | 'session_council'
  | 'ignore';
export type ExternalWorkLinkRole = 'primary' | 'derived';
...
export type UpsertExternalWorkItemInput = {
  userId: string;
  source: ExternalWorkSource;
  externalId: string;
  identifier: string;
...
export type UpdateExternalWorkItemInput = {
  itemId: string;
  userId: string;
  triageStatus?: ExternalWorkTriageStatus;
  lastSyncedAt?: string | null;
...
export type CreateExternalWorkLinkInput = {
  externalWorkItemId: string;
  targetType: ExternalLinkTargetType;
  targetId: string;
  role: ExternalWorkLinkRole;
...
export type WorkflowPollingConfig = {
  intervalMs: number;
  batchSize: number;
...
export type WorkflowAgentConfig = {
  ...
  autoApproveMainCommand: boolean;
};

export type WorkflowPullRequestConfig = {
  draft: boolean;
  branchPrefix: string;
...
export type WorkflowCodexConfig = {
  ...
  pullRequest: WorkflowPullRequestConfig;
};
...
export type ExecutionGraphSource = 'planner' | 'runner_workflow' | 'legacy_projection';
...
export type ToolInvocationSource = 'internal' | 'mcp' | 'openapi';
...
export type ToolPolicyDecision = {
  source: ToolInvocationSource;
  name: string;
  disposition: 'allow' | 'approval_required' | 'deny';
  rationale: string;
...
export type CreateRunnerRunInput = {
  id?: string;
  userId: string;
  workItem: WorkItem;
  claimState?: RunnerClaimState;
...
export type UpdateRunnerRunInput = {
  runId: string;
  userId?: string;
  claimState?: RunnerClaimState;
  status?: RunnerRunStatus;
...
export type UpsertRunnerStateInput = {
  dispatchEnabled?: boolean;
  refreshRequestedAt?: string | null;
  refreshedAt?: string | null;
  workflowPath?: string | null;
...
export type JarvisStore = {
  ...
  upsertExternalWorkItems: (input: {
    items: UpsertExternalWorkItemInput[];
  }) => Promise<ExternalWorkItemRecord[]>;
  listExternalWorkItems: (input: {
    userId: string;
    source?: ExternalWorkSource;
    triageStatus?: ExternalWorkTriageStatus;
    limit: number;
  }) => Promise<ExternalWorkItemRecord[]>;
  getExternalWorkItemById: (input: {
    itemId: string;
    userId: string;
  }) => Promise<ExternalWorkItemRecord | null>;
  getExternalWorkItemBySource: (input: {
    userId: string;
    source: ExternalWorkSource;
    externalId: string;
  }) => Promise<ExternalWorkItemRecord | null>;
  updateExternalWorkItem: (input: UpdateExternalWorkItemInput) => Promise<ExternalWorkItemRecord | null>;
  createExternalWorkLink: (input: CreateExternalWorkLinkInput) => Promise<ExternalWorkLinkRecord>;
  listExternalWorkLinksByItem: (input: { itemId: string }) => Promise<ExternalWorkLinkRecord[]>;
  listExternalWorkLinksByTarget: (input: {
    targetType: ExternalLinkTargetType;
    targetId: string;
  }) => Promise<ExternalWorkLinkRecord[]>;
  getPrimaryExternalWorkLinkByItem: (input: { itemId: string }) => Promise<ExternalWorkLinkRecord | null>;
  getPrimaryExternalWorkLinkByTarget: (input: {
    targetType: ExternalLinkTargetType;
    targetId: string;
  }) => Promise<ExternalWorkLinkRecord | null>;
  getRunnerState: () => Promise<RunnerStateRecord>;
  upsertRunnerState: (input: UpsertRunnerStateInput) => Promise<RunnerStateRecord>;
  createRunnerRun: (input: CreateRunnerRunInput) => Promise<RunnerRunRecord>;
  listRunnerRuns: (input: {
    userId?: string;
    status?: RunnerRunStatus;
    limit: number;
  }) => Promise<RunnerRunRecord[]>;
  getRunnerRunById: (input: { runId: string; userId?: string }) => Promise<RunnerRunRecord | null>;
  findActiveRunnerRunByWorkItem: (input: {
    source: WorkItemSource;
    identifier: string;
  }) => Promise<RunnerRunRecord | null>;
  updateRunnerRun: (input: UpdateRunnerRunInput) => Promise<RunnerRunRecord | null>;
